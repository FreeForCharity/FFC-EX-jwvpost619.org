...
describe('root layout consent bootstrap ordering', () => {
  // The layout is a server component excluded from jest rendering, so assert
  // on its source. This fork's layout loads NO Google tag component — the
  // bootstrap is inert groundwork — so the invariant is: the bootstrap is
  // imported and emitted inside <head>, ahead of any future Google tag.
  ...
  it('emits the bootstrap script inside <head>', () => {
    ...
    expect(bootstrapMatch!.index).toBeLessThan(headClose)
  })
})
